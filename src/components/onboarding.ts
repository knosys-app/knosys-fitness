...
function estimateCalories(
  ...
  // Adjust for weight goal: deficit to lose, surplus to gain
...
export function createOnboarding(Shared: SharedDependencies) {
  ...
  return function Onboarding({ onComplete }: { onComplete: () => void }) {
    ...
    const handleFinish = async () => {
      ...
      const age = birthDate ? estimateAge(birthDate) : 30;
      ...
      const fatG = Math.round((calories * 0.25) / 9); // 25% from fat
      const carbsG = Math.round((calories - proteinG * 4 - fatG * 9) / 4); // remainder from carbs
      ...
      React.createElement(Card, { className: 'w-full max-w-md' },
        ...
        React.createElement('div', { className: 'flex justify-center gap-6 px-6 pb-2' },
          ...steps.map((s, i) => {
            ...
            return React.createElement('div', {
              key: i,
              className: cn('flex flex-col items-center gap-1', active ? 'text-primary' : done ? 'text-primary/60' : 'text-muted-foreground/40'),
            },
        ...
        React.createElement(CardContent, { className: 'pt-4 space-y-4' },
