import type { SharedDependencies, PluginAPI, MealType, MealLog, Goals, FoodEntry, NormalizedFood, Exercise, ExerciseEntry, ExerciseLog, WaterEntry, WeightEntry, FitnessSettings, UserProfile } from '../types';
...
/** Hook for exercise catalog search. Local catalog + custom exercises (S7 adds remote sources). */
export function createUseExerciseSearch(Shared: SharedDependencies) {
  ...
  return function useExerciseSearch() {
    ...
    const search = useCallback(async (q: string) => {
      ...
      // Remote sources only fire when user has typed a query (avoid hammering APIs on empty state).
      if (trimmed) {
      ...
      // Dedupe by normalized name + equipment; preference order: custom > yuhonas > wger > api-ninjas.
    ...
    // Re-run search when source or catalog readiness changes.
